...
//
// ATTENDANCE SOURCE (FINAL / PRODUCTION):
//   third_party_attendance_summary
//   columns: id, employee_id, client_id, deployment_id, billing_month,
//            total_days, working_days, present_days, absent_days,
//            leave_days, half_days, lop_days, payable_days,
//            overtime_hours, created_at, updated_at
//
// third_party_payroll has NO "status" column.
// Do not select, update, or reference "status" anywhere in this file.
// ============================================================
...
const ATTENDANCE_COLUMNS = `
    ...
    total_days, working_days, present_days, absent_days, leave_days,
    half_days, lop_days, payable_days, overtime_hours,
...
function getDaysInMonth(salaryMonth) {
    const [year, monthNumber] = validateSalaryMonth(salaryMonth).split("-").map(Number);
...
// ============================================================
// ATTENDANCE LOOKUPS (third_party_attendance_summary)
// ============================================================

// Used when we already have a payroll row (GET list / GET single)
async function getAttendanceForPayroll(payroll) {
    let attendance = null;

    // 1) exact match via stored attendance_id (fastest, most precise)
    if (payroll?.attendance_id) {
        const { data, error } = await supabase
            .from(ATTENDANCE_TABLE)
            .select(ATTENDANCE_COLUMNS)
            .eq("id", payroll.attendance_id)
            .maybeSingle();
        if (error) throw error;
        attendance = data;
    }

    // 2) employee + deployment + month
    if (!attendance && payroll?.employee_ref_id && payroll?.deployment_id && payroll?.salary_month) {
        const { data, error } = await supabase
            .from(ATTENDANCE_TABLE)
            .select(ATTENDANCE_COLUMNS)
            .eq("employee_id", payroll.employee_ref_id)
            .eq("deployment_id", payroll.deployment_id)
            .eq("billing_month", payroll.salary_month)
            .order("id", { ascending: false })
            .limit(1);
        if (error) throw error;
        attendance = data?.[0] || null;
    }

    // 3) employee + month only (fallback)
    if (!attendance && payroll?.employee_ref_id && payroll?.salary_month) {
        const { data, error } = await supabase
            .from(ATTENDANCE_TABLE)
            .select(ATTENDANCE_COLUMNS)
            .eq("employee_id", payroll.employee_ref_id)
...
// Used by /generate: every attendance summary row for a client+month
// (each row already carries its own deployment_id, so this drives
// which employees get payroll generated).
async function getAttendanceRowsForClientMonth(clientId, salaryMonth) {
    const { data, error } = await supabase
        .from(ATTENDANCE_TABLE)
        .select(ATTENDANCE_COLUMNS)
...
// Single lookup by employee + deployment + month (used by POST / and lookup/prefill)
async function getAttendanceByEmployeeDeploymentMonth(employeeId, deploymentId, salaryMonth) {
    const { data, error } = await supabase
        .from(ATTENDANCE_TABLE)
        .select(ATTENDANCE_COLUMNS)
...
function formatPayroll(row, attendance = null) {
    ...
    return {
        ...
        working_days: num(attendance?.working_days),
        ...
        attendance_updated_at: attendance?.updated_at || null,
    ...
    try {
        ...
        for (const row of data || []) {
            try {
                ...
                console.error(`Attendance enrichment failed for payroll ${row.id}:`, err.message);
...
// ============================================================
// GET SINGLE PAYROLL
// GET /api/payroll/:id   (kept out of the way of /generate, /lookup/*)
// ============================================================
...
// ============================================================
// GENERATE PAYROLL (bulk, for a whole client + month)
// POST /api/payroll/generate   { client_id, salary_month }
//
// Driven entirely by third_party_attendance_summary rows —
// one payroll record per attendance summary row that doesn't
// already have payroll generated.
// ============================================================
...
router.post("/generate", async (req, res) => {
    try {
        ...
        const attendanceRows = await getAttendanceRowsForClientMonth(clientId, validMonth);

        if (attendanceRows.length === 0) {
            return res.json({
                success: true,
                message: `No attendance summary found for ${client.company_name} in ${validMonth}`,
                client_id: clientId,
        ...
        for (const attendance of attendanceRows) {
            const employeeId = getId(attendance.employee_id);
            const deploymentId = getId(attendance.deployment_id);

            try {
                if (!employeeId) {
                    skipped.push({ attendance_id: attendance.id, reason: "Employee ID missing" });
                    continue;
                }
                if (!deploymentId) {
                    skipped.push({ attendance_id: attendance.id, employee_id: employeeId, reason: "Deployment ID missing" });
                    continue;
                }
                ...
                if (!candidate) {
                    skipped.push({ attendance_id: attendance.id, employee_id: employeeId, reason: "Candidate not found" });
                    continue;
                }
                ...
                console.error(`Payroll generation failed for attendance ${attendance.id}:`, err);
                skipped.push({ attendance_id: attendance.id, employee_id: employeeId, reason: err.message });
            }
        }
...
router.post("/", async (req, res) => {
    try {
        ...
        if (!(num(candidate.pay_rate) > 0)) {
            return sendError(res, 400, "Basic Salary is not configured for this employee in candidates.pay_rate");
        }
        ...
        if (existingRows && existingRows.length > 0) {
            return sendError(res, 409, "Payroll already exists for this employee, deployment and month", {
                existing_payroll_id: existingRows[0].id,
            });
        }

        const attendance = await getAttendanceByEmployeeDeploymentMonth(employeeId, deploymentId, salaryMonth);

        if (!attendance) {
            return sendError(res, 400, "No attendance summary found for this employee, deployment and month");
        }
        ...
        return res.status(201).json({
            ...
            data: {
                ...
                attendance: {
                    ...
                    total_days: num(attendance.total_days),
                    working_days: num(attendance.working_days),
                    present_days: num(attendance.present_days),
                    absent_days: num(attendance.absent_days),
                    leave_days: num(attendance.leave_days),
...
// (No status/lock concept — any record can be corrected)
// ============================================================
...
router.patch("/:id", async (req, res) => {
    try {
        ...
        const numberValue = (field, fallback) => {
            ...
            if (raw === undefined || raw === null || raw === "") return num(fallback);
        ...
        const updateData = {
            ...
            total_employer_cost: r2(grossSalary + employerContribution + num(payroll.gratuity)),
        ...
        return res.json({ success: true, message: "Payroll updated successfully", data: updatedPayroll });
    } catch (error) {
...
router.post("/:id/email", async (req, res) => {
    try {
        ...
        if (!EMAIL_USER || !EMAIL_PASS) {
            ...
            return sendError(res, 500, "Email service is not configured. Please check EMAIL_USER and EMAIL_PASS.");
        }
        ...
        if (!candidateId) return sendError(res, 400, "Employee reference ID is missing from payroll record.");
        ...
        if (!employeeUser) return sendError(res, 404, `Employee account not found for employee ID ${candidateId}.`);

        const employeeEmail = String(employeeUser.email || "").trim();
        const employeeName = candidate.full_name || employeeUser.name || payroll.employee_name || "Employee";

        if (!employeeEmail) return sendError(res, 400, `Employee email address is not available for ${employeeName}.`);
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(employeeEmail)) {
            return sendError(res, 400, `Invalid employee email address: ${employeeEmail}`);
        }

        const salaryMonth = payroll.salary_month || new Date().toISOString().slice(0, 7);

        const numbers = Object.fromEntries(
            [...NUMERIC_FIELDS, "hra", "conveyance", "medical_allowance", "other_allowance", "gratuity", "pf_wages"].map(
                (f) => [f, num(payroll[f])]
            )
        );
        ...
        if (!pdfBuffer) throw new Error("Payslip PDF generation returned no data.");

        const cleanName = String(employeeName).replace(/[^a-zA-Z0-9]+/g, "_").replace(/^_+|_+$/g, "");
        ...
        const safeName = String(employeeName).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
router.get("/:id/pdf", async (req, res) => {
    try {
        ...
        const { data: candidate, error: cErr } = await supabase
            .from("candidates")
            .select("id, full_name, designation, employee_code, date_of_joining, city, pan_number, uan_number, esic_number, department")
        ...
        const cleanName = String(data.employee_name || "Employee").replace(/[^a-zA-Z0-9]+/g, "_").replace(/^_+|_+$/g, "");

        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename="Payslip_${cleanName}_${data.salary_month}.pdf"`);
...
router.get("/lookup/employees", async (req, res) => {
    try {
        ...
        if (deploymentError) throw deploymentError;
        if (!deployments || deployments.length === 0) return res.json({ success: true, data: [] });

        const candidateIds = [...new Set(deployments.map((d) => d.candidate_id).filter((v) => v != null))];
...
router.get("/lookup/prefill", async (req, res) => {
    try {
        ...
        const attendance = await getAttendanceByEmployeeDeploymentMonth(deployment.candidate_id, deploymentId, salaryMonth);
        ...
        const payableDays = Math.max(
            0,
            Math.min(
                attendance?.payable_days != null ? num(attendance.payable_days) : calendarDays - lopDays - halfDays * 0.5,
                calendarDays
            )
        );
        ...
        return res.json({
            ...
            data: {
                ...
                total_days: num(attendance?.total_days) || calendarDays,
                working_days: num(attendance?.working_days),
                present_days: num(attendance?.present_days),
                absent_days: num(attendance?.absent_days),
                leave_days: num(attendance?.leave_days),
